...
# Step 1: Prepare the data — monthly counts
...
monthly_long <- monthly_counts %>%
  pivot_longer(cols = c(food_count, clinical_count),
...
ggplot(monthly_long, aes(x = month, y = count, color = source_type)) +
  ...
  scale_color_manual(values = c("food_count" = "forestgreen", "clinical_count" = "firebrick")) +
  theme_minimal()
```
...
Isolatecounts <- ggplot(monthly_long, aes(x = month, y = count, fill = source_type)) +
  ...
  scale_fill_manual(values = c("food_count" = "forestgreen", "clinical_count" = "firebrick")) +
  theme_minimal() +
